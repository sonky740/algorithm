...
function solution(s) {
  ...
  while (s !== '1') {
    const zero = s.match(/0/g) ? s.match(/0/g).length : 0;
    const one = s.match(/1/g) ? s.match(/1/g).length : 0;
    s = one.toString(2);
    answer[0] = answer[0] ? answer[0] + 1 : 1;
    answer[1] = answer[1] ? answer[1] + zero : zero;
  }
  return answer;
}
