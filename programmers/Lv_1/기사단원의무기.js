...
function solution(number, limit, power) {
  ...
  // 각 기사의 약수 개수 구하기
  // 하나의 약수를 알면 다른 하나의 존재가 보장되므로, 제곱근만 따로 구하면 된다.
  numberList.map((n) => {
    divisorObj[n] = 0;
    for (let i = 1; i * i <= n; i++) {
      if (i * i === n) divisorObj[n]++;
      else if (n % i === 0) divisorObj[n] += 2;
    }
  });

  // 약수 개수를 배열로 만들기
  for (let key in divisorObj) {
    divisorArr.push(divisorObj[key]);
  }

  excessArr = divisorArr.filter((el) => el > limit);
  divisorArr = divisorArr.filter((el) => el <= limit);

  answer = divisorArr.reduce((a, b) => a + b) + excessArr.length * power;

  console.log(divisorArr, excessArr);
  return answer;
}
